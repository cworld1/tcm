...
class ComboCheckBox(QComboBox):
    def __init__(self, items):  # items==[str,str...]
        ...
        for i in range(1, self.row_num):
            self.addQCheckBox(i)
            self.qCheckBox[i].stateChanged.connect(lambda: self.isshow(self.items[i])) #槽函数关联 函数是show（）
...
def Kinect_Capture():
    ...
    while (True):
        ...
        if depth_image_handle and color_image_handle:
            ...
            depth_image = pyK4A.transform_depth_to_color(depth_image_handle, color_image_handle)
    ...
    if i <= 2:
        ...
        for j in range((len(AcupointsPosition[0][i]) - 2) / 2):
            ...
            image = cv.line(image, AcupointsPosition[1][i][j + len(AcupointsPosition[1][i])],
                            AcupointsPosition[1][i][j + 1 + len(AcupointsPosition[1][i])], color=(255, 0, 3),
                            thickness=2)
...
def GUI(QWidget):
    ...
    def __init__(self):
        ...
        self.meridiansButtons = ComboCheckBox(meridians)
