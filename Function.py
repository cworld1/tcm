...
Name = [[["天泉（左）", "曲泽（左）", "郄门（左）", "间使（左）", "内关（左）", "大陵（左）", "天泉（右）", "曲泽（右）", "郄门（右）",
        ...
        [["肩髎(左)", "臑会(左)", "天井(左)", "消泺(左)", "四渎(左)", "三阳络(左)", "支沟(左)", "外关(左)", "阳池(左)",
          "肩髎(右)", "臑会(右)", "天井(右)", "消泺(右)", "四渎(右)", "三阳络(右)", "支沟(右)", "外关(右)", "阳池(右)"],
    ...
    if Pose_Landmarks:
        ...
        if Pose_Landmarks[12][2] < Pose_Landmarks[24][2]:
            if Pose_Landmarks[12][1] < Pose_Landmarks[11][1]:
                ...
                AcupointsPosition[1][0][2] = (int(cx13), int(cy13))
                AcupointsPosition[1][0][3] = (int((cx13 * 2 + cx11) // 3), int((cy13 * 2 + cy11) // 3))
                ...
                AcupointsPosition[1][0][11] = (int(cx14), int(cy14))
                AcupointsPosition[1][0][12] = (int((cx14 * 2 + cx12) // 3), int((cy14 * 2 + cy12) // 3))
...
while (True):
    ...
    if depth_image_handle and color_image_handle:
        ...
        print(xy[0], xy[1])
        x, y, z = Projection(640.612671, 367.137726, 607.669800, 607.552429, xy[1], xy[0], depth_color_image[xy[1]][xy[0]])

        print("x: " + str(x) + '   y: ' + str(y) + '   z: ' + str(z))

        cv.circle(cv.Mat(color_image), xy, 7, (255, 0, 255), cv.FILLED)
